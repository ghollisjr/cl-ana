...
;;; http://www.hdfgroup.org/ftp/HDF5/examples/examples-by-api/hdf5-examples/1_8/C/H5T/h5ex_t_opaqueatt.c
...
(defparameter *FILE*    (namestring (merge-pathnames "h5ex_t_opaqueatt.h5" *load-pathname*)))
...
(let* ((fapl (h5pcreate +H5P-FILE-ACCESS+))
  ...
  (unwind-protect
       (let* ((wdata (create-wdata *DIM0*))
              ...
              (attr (h5acreate2 dset *ATTRIBUTE* dtype aspace +H5P-DEFAULT+
                                +H5P-DEFAULT+)))
         (h5awrite attr dtype wdata)
         (h5ex:close-handles (list attr aspace dset dspace dtype))
  ...
  (unwind-protect
	   ...
	   (let ((dims[0] (cffi:mem-aref dims 'hsize-t 0)))
	     (cffi:with-foreign-object (rdata :char (* dims[0] len))
	       ;; Read the data.
	       (h5aread attr dtype rdata)
	       ;; Output the data to the screen.
               (format t "Datatype tag for ~a is: \"~a\"~%"
                       *ATTRIBUTE* (cffi:foreign-string-to-lisp tag))
               (dotimes (i dims[0])
